...
class Flux2Trainer(Flux2NetworkTrainer):
    ...
    def train(self, args):
        ...
        # Flux2NetworkTrainer sets args.dit_dtype to mixed precision, override here for full finetune (float32 or bfloat16 with --full_bf16)
        ...
        if args.sdpa:
            ...
            raise ValueError(
                "either --sdpa, --flash-attn, --flash3 or --xformers must be specified / --sdpa, --flash-attn, --flash3, --xformersのいずれかを指定してください"
            )
        ...
        # single param group: train all DiT parameters
        params_to_optimize = []
        ...
        # experimental feature: train the model in bf16
        ...
        # calculate epoch count
        num_update_steps_per_epoch = math.ceil(len(train_dataloader) / args.gradient_accumulation_steps)
        num_train_epochs = math.ceil(args.max_train_steps / num_update_steps_per_epoch)

        accelerator.print("running training / 学習開始")
        accelerator.print(f"  num train items / 学習画像数: {train_dataset_group.num_train_items}")
...
def flux2_finetune_setup_parser(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    """FLUX.2 full fine-tuning specific parser setup"""
    parser.add_argument("--full_bf16", action="store_true", help="Enable full bfloat16 training (DiT weights stored and updated in bf16). Requires --mixed_precision bf16. / bfloat16でDiTの重みを保持・更新します。--mixed_precision bf16が必要です。")
    parser.add_argument("--fused_backward_pass", action="store_true", help="Use fused backward pass for Adafactor optimizer (saves memory). / Adafactorオプティマイザー向けのfused backward passを使用します（メモリ節約）。")
    parser.add_argument(
        "--mem_eff_save",
        action="store_true",
    ...
    parser.add_argument(
        ...
        + " / --blocks_to_swapを使用時にoptimizerパラメータのデバイスをパッチします。一部のoptimizerでのみ動作します。--fused_backward_passを使用しているときは指定不要です。",
    )
    return parser
...
def main():
    ...
    if args.fp8_base or args.fp8_scaled:
        logger.warning(
            "FP8 training is not supported for fine-tuning. Set --fp8-base or --fp8-scaled to False."
        )
        args.fp8_base = False
